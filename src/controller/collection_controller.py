...
class CollectionController:
    ...
    def __init__(self, settingsModel: SettingsModel, file_util: FileUtils,ColUtils:CollectionUtils):
        ...
        self.__scenes_list = self.ColUtils.get_scenes_list(settingsModel.nuscenes, self.settings_model.scene_for_eval_set)
    ...
    def run(self):
        ...
        if settings_model.goal == GOAL.goal1:
            self._compute_goal1()
        else:
            self._compute_goal2()
        ...
        data = self.file_util.read_data_from_file(
            settings_model.save_build_precision_recall_curve_data + "metrics_details.json")
        special_05,special_1,special_2,special_4= self.ColUtils.plot_special(data,confidence_threshold,save_in,d,r,t)
        self.ColUtils.plot_normal_and_special( special_05, special_1, special_2, special_4, normal_05, normal_1, normal_2,
                                normal_4, save_in, confidence_threshold, self.settings_model.detector.value, d, r, t)
    ...
    def _compute_selected_precision_recall(self, critic=None, save_in=None):
        ...
        dt = self.ColUtils.create_dt(nuscenes=settings_model.nuscenes, conf_value=settings_model.conf_value,
                                detector_file=settings_model.detector_file, val='val', model=detector.value, d=d, r=r,
    ...
    def _compute_goal2(self):
        ...
        for detector_name, folder in detectors.items():
            ...
            self.file_util.write_data_to_file(output_folder_path=output_folder + 'pkl_crit_results_GOAL2.json',
                                              key="PKL_CRIT_RESULTS_GOAL2",
    ...
    def _iterate_drt_goal2(self, detector_name, detector_file, output_folder):
        ...
        for d, r, t in drt:
            ...
            pkl_crit_results_store.append(pkl_crit_results)
            self.file_util.write_data_to_file(output_folder_path=output_folder + 'pkl_crit_results_GOAL2.json',
                                              key="PKL_CRIT_RESULTS_GOAL2", value=pkl_crit_results_store)
