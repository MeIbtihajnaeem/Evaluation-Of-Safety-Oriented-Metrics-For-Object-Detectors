...
class EvalBox(abc.ABC):
    ...
    MAX_DISTANCE_OBJ: float = 8888.0
    MAX_DISTANCE_INTERSECT: float = 8888.0
    MAX_TIME_INTERSECT_OBJ: float = 8888.0
...
class EvalBoxes:
    ...
    @classmethod
    def deserialize(cls, content: dict, box_cls, nusc):
        """
        ...
        for sample_token, boxes in content.items():
            eb.add_boxes(sample_token, [box_cls.deserialize(box, nusc) for box in boxes])
        return eb
