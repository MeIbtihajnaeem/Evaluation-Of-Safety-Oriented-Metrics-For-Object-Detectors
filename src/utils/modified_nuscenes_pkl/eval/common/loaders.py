...
from ..detection.consolidated_imports import import_utils, import_data_classes

Box = import_all()
category_to_detection_name = import_utils()
DetectionBox = import_data_classes()


#Done,Done

def load_prediction(nusc: NuScenes, result_path: str, max_boxes_per_sample: int, box_cls, verbose: bool = False) \
    ...
    if 'results' not in data:
        raise ValueError('Error: No field `results` in result file. Please note that the result format changed. '
                         'See https://www.nuscenes.org/object-detection for more information.')
    # Deserialize results and get meta data.
    all_results = EvalBoxes.deserialize(data['results'], box_cls, nusc)
    meta = data['meta']
    if verbose:
    ...
    for sample_token in all_results.sample_tokens:
        if len(all_results.boxes[sample_token]) > max_boxes_per_sample:
            raise ValueError("Error: Only <= %d boxes per sample allowed!" % max_boxes_per_sample)
...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = True) -> EvalBoxes:
    ...
    """
    # Init.
    if box_cls == DetectionBox:  # QUI ORA ENTRA, MIRACOLOSAMENTE
        attribute_map = {a['token']: a['name'] for a in nusc.attribute}
    else:
        box_cls = DetectionBox
        print("loaders.py: WARNING: ho forzato l'equivalenza box_cls == DetectionBox")
        attribute_map = {a['token']: a['name'] for a in nusc.attribute}
        sys.exit(0)
    ...
    if len(sample_tokens_all) == 0:
        raise ValueError("Error: Database has no samples!")
    ...
    if eval_split in {'train', 'val', 'train_detect', 'train_track'}:
        if not version.endswith('trainval'):
            raise ValueError(
                'Error: Requested split {} which is not compatible with NuScenes version {}'.format(eval_split,
                                                                                                    version))
    elif eval_split in {'mini_train', 'mini_val'}:
        if not version.endswith('mini'):
            raise ValueError(
                'Error: Requested split {} which is not compatible with NuScenes version {}'.format(eval_split,
                                                                                                    version))
    else:
