...
# from ..common.utils import center_distance, scale_iou, yaw_diff, velocity_l2, attr_acc, cummean
...
DetectionMetricData = detection_data_classes()
...
def accumulate(gt_boxes: EvalBoxes,
    ...
    for i in npos_crit1:
        if (
                i > 0.0):  # TODO: il problema è nei NaN in data_classes, dovuti alle divisioni per 0. Qualcosa da aggiustare nelle formule.
    ...
    tp_gt_crit = np.cumsum(tp_gt_crit).astype(
        float)  # TODO: il problema è nei NaN in data_classes, dovuti alle divisioni per 0. Qualcosa da aggiustare nelle formule.
    ...
    if (recall_type == "GT AL NUMERATORE"):
        prec_crit = tp_pred_crit / (
                fp_pred_crit + tp_pred_crit)  # TODO: ha dato 1 warning su runtime value not valid???
