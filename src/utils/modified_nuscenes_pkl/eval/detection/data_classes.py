...
# no constants file in the detection therefore importing from library
from nuscenes.eval.detection.data_classes import DetectionConfig
from nuscenes.eval.detection.constants import DETECTION_NAMES, ATTRIBUTE_NAMES, TP_METRICS
# from ..common.consolidated_imports import import_data_classes
from ..common.data_classes import MetricData, EvalBox
...
class DetectionMetricData(MetricData):
    ...
    def deserialize(cls, content: dict, nusc):
        ...
        return cls(recall=np.array(content['recall']),
                   ...
                   recall_crit=None, precision_crit=None)
...
class DetectionMetrics:
    ...
    @property
    def mean_ap_crit(self) -> float:
        """ Calculates the mean AP by averaging over distance thresholds and classes. """
        return float(np.mean(list(self.mean_dist_aps_crit.values())))
    ...
    def serialize(self):
        return {
            ...
            'label_aps_crit': self._label_aps_crit,
            'mean_dist_aps': self.mean_dist_aps,
            'mean_ap': self.mean_ap,
            'mean_ap_crit': self.mean_ap_crit,
    ...
    def deserialize(cls, content: dict, nusc):
        ...
        for detection_name, label_aps_crit in content['label_aps_crit'].items():
            for dist_th, ap in label_aps_crit.items():
                metrics.add_label_ap_crit(detection_name=detection_name, dist_th=float(dist_th), ap_crit=float(ap_crit))
    ...
    def __eq__(self, other):
        ...
        eq = eq and self._label_aps_crit == other._label_aps_crit
...
class DetectionBox(EvalBox):
    ...
    def __init__(self,
                 ...
                 ego_translation: [float, float, float] = (0, 0, 0),
                 # Translation to ego vehicle in meters. NOT USED????
        ...
        if (math.isnan(v_Bx_egox) or math.isnan(v_By_egoy)):  # CAPITA MA NON E' COSA CHE POSSIAMO RISOLVERE NOI
            ...
            C_x: float = ((v_By_egoy ** 2) * B_X + (
                    v_Bx_egox ** 2) * ego_x - v_Bx_egox * v_By_egoy * B_Y + v_Bx_egox * v_By_egoy * ego_y) / (
                                 v_Bx_egox ** 2 + v_By_egoy ** 2)
            C_y: float = ((v_Bx_egox ** 2) * B_Y + (v_By_egoy ** 2) * ego_y + v_Bx_egox * v_By_egoy * (ego_x - B_X)) / (
                    v_Bx_egox ** 2 + v_By_egoy ** 2)
...
class DetectionMetricDataList:
    ...
    def deserialize(cls, content: dict, nusc):
        ...
        for key, md in content.items():
            ...
            mdl.set(name, float(distance), DetectionMetricData.deserialize(md, nusc=nusc))
        return mdl
